      ...
      <div className="flex-1 px-4 py-4 flex flex-col gap-5 overflow-y-auto">
        ...
        <section>
              ...
              {game.hs_activity_id && (
                <button
                  onClick={async () => {
                    ...
                    setHsSyncMsg('');
                    try {
                      const res = await api.post<{ added: number; skipped: number; total: number }>(
                        '/holdsport/sync-game-players', { game_id: game.id }
                      );
                      await fetchRoster();
                      setHsSyncMsg(res.added > 0 ? `+${res.added} tilmeldte` : 'Ingen nye');
                      setTimeout(() => setHsSyncMsg(''), 3000);
                    } catch (e) {
                      setHsSyncMsg(e instanceof Error ? e.message : 'Fejl');
                      setTimeout(() => setHsSyncMsg(''), 4000);
                    } finally {
                      setHsSyncing(false);
                    }
                  }}
                  disabled={hsSyncing}
                  className="flex items-center gap-1 text-xs font-semibold text-text2 disabled:opacity-40"
                  title="Synk tilmeldte fra Holdsport"
                >
                  {hsSyncing
                    ? <div className="w-3.5 h-3.5 border-2 border-text3 border-t-transparent rounded-full animate-spin" />
                    : <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>
                  }
                  {hsSyncing ? 'Synker…' : 'Holdsport'}
                </button>
              )}
              <button
          ...
          {roster.filter(r => r.coach_id).map(entry => (
            <div key={entry.id} className="bg-bg rounded-xl border border-border flex items-center gap-3 px-3 py-2.5 mb-1.5">
              <div className="w-8 h-8 rounded-full bg-bg2 flex items-center justify-center shrink-0">
                <span className="text-xs font-bold text-text2">
                  {(entry.coach_name ?? '?').split(' ').map((w: string) => w[0]).slice(0, 2).join('').toUpperCase()}
                </span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-text1 truncate">{entry.coach_name}</p>
                <p className="text-[10px] text-text3">Træner</p>
              </div>
              <button
          ...
          {roster.filter(r => r.player_id).length === 0 && roster.filter(r => r.coach_id).length === 0 ? (
